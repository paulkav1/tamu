...
<body>	
	...
	<h1>Looked for ${params.firstName} ${params.lastName} ${params.phone} ${params.email} 
	<g:link action="insert" params="[id:"${params.id}"]">Insert Lead</g:link></h1>
	<table>
		<thead>
			<tr>
		...
		<tbody>
			<g:each in="${sugar}" status="i" var="sug">
				<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">								
					<td><g:link action="sugar" id="${sug.sid}">Show</g:link> <g:link controller="applicant" action="updateCid" params="[id:"${params.id}", colloquyId:"${sug.sid}"]" id="${params.id}">Update</g:link></td>
					<td>${sug.sid}</td>
